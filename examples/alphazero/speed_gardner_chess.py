...
DEFAULT_TIME = 300  # "ticks" per player for speed chess
...
class State(core.State):
    ...
    # --- Speed-chess time control (per RL player: 0/1) ---
    # Internal name so pgx API tests treat it as private.
...
class GardnerChess(core.Env):
    ...
    def _init(self, key: PRNGKey) -> State:
        # Randomly choose which RL player moves first.
    ...
    def step(
        ...
        # PGX legality check (must happen before _step)
    ...
    # ------------------------------------------------------------------
    def _step(
        self, state: core.State, action_input, key
        ...
        # Subtract time for current RL player (before flipping to opponent)
        idx = state.current_player  # 0 or 1
        new_time = state._time_left.at[idx].add(-time_spent)
        time_over = new_time[idx] <= 0

        # Apply the Gardner-chess move and board-based termination/reward
        state = _step_board(state, action)

        # Base termination and rewards from Gardner rules
        base_termination = state.terminated
        base_rewards = state.rewards

        # Time-out overrides board result: the player who flags loses.
    ...
    def version(self) -> str:
        # We keep "v0" for compatibility; semantics now include time control.
        return "v0"
...
# ----------------------------------------------------------------------
# Core game logic (unchanged, except _step → _step_board)
# ----------------------------------------------------------------------
def _step_board(state: State, action: Array) -> State:
    """Pure board + rule step (no time control)."""
...
def _apply_move(state: State, a: Action):
    ...
    state = state.replace(  # type: ignore
        ...
        _fullmove_count=state._fullmove_count
        + jnp.int32(state._turn == 1),
    )
...
def _observe(state: State, player_id: Array):
    ...
    no_prog_cnt = (
        state._halfmove_count.astype(jnp.float32) / 100.0
    ) * ones
...
def _possible_piece_positions(state: State):
    ...
    opp_pos = jnp.nonzero(
        _flip(state)._board > 0, size=10, fill_value=-1
    )[0].astype(jnp.int32)
...
def _flip(state: State) -> State:
    return state.replace(  # type: ignore
        ...
        _board_history=-jnp.flip(
            state._board_history.reshape(8, 5, 5), axis=-1
        ).reshape(-1, 25),
