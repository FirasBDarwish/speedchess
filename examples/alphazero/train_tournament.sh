#!/bin/bash
#SBATCH --job-name=alphazero_tournament                   # Default job name (sweep overrides)
...
#SBATCH -o slurm-%x-%j_alphazero_tournament.out                 # STDOUT (%x=jobname, %j=jobid)
#SBATCH -e slurm-%x-%j_alphazero_tournament.err                 # STDERR
...
~/.conda/envs/torch/bin/python tournament_mcts.py
