#!/bin/bash
#SBATCH --job-name=alphazero_obs_gate_jit                   # Default job name (sweep overrides)
...
#SBATCH -o slurm-%x-%j_alphazero_obs_gate_jit.out                 # STDOUT (%x=jobname, %j=jobid)
#SBATCH -e slurm-%x-%j_alphazero_obs_gate_jit.err                 # STDERR
...
~/.conda/envs/torch/bin/python train_gate_speed_gardner_jit.py
