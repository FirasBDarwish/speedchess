...
#     # Note: Pydantic might not like the '+' in '+save_suffix', 
#     # you might need to clean the input dict before passing it, 
#     # or ensure the variable name matches exactly what is passed.
...
#     suffix = config.save_suffix if config.save_suffix else ""
#     ckpt_dir = os.path.join("checkpoints", f"{config.env_id}{suffix}_{now}")
...
class Config(BaseModel):
    # Pydantic V2 Configuration: Ignore extra fields instead of crashing
    ...
    # Added these fields because they are used in the script
...
# CLEANING STEP: Remove '+' from keys (e.g. +save_suffix -> save_suffix)
# This allows Hydra-style overrides to correctly map to Pydantic fields.
cleaned_conf = {}
for k, v in conf_dict.items():
    clean_k = k.lstrip('+') if isinstance(k, str) else k
    cleaned_conf[clean_k] = v

config: Config = Config(**cleaned_conf)
print(config)
