...
class Config(BaseModel):
    ...
    sim_tick_cost: int = 5
    # Used to set initial time per player: initial_time = num_simulations * sim_tick_cost * avg_moves
    avg_moves: int = 70  # NOTE: this is huge; expect many truncations if you leave it here
...
INITIAL_TIME = config.num_simulations * config.sim_tick_cost * config.avg_moves  # ticks
MOVE_TICK_COST = config.num_simulations * config.sim_tick_cost  # ticks per move
BASELINE_TICK_COST = config.baseline_tick_per_move  # baseline per move


def init_with_custom_time(key: jnp.ndarray):
    """Wrapper around env.init that sets a larger per-player time budget."""
    state = env.init(key)
...
def selfplay(model, rng_key: jnp.ndarray):
    """Run batched self-play with fixed num_simulations and time-cost per move.
    ...
    move_time = jnp.int32(MOVE_TICK_COST)
    ...
    def step_fn(state, key) -> tuple[pgx.State, SelfplayOutput]:
        ...
        # All real moves during self-play pay the same time cost per move.
        time_spent = jnp.full((batch_size,), move_time, dtype=jnp.int32)

        # Auto-reset terminated environments, and step with (action, time_spent)
        state = jax.vmap(step_with_auto_reset)(
            state, (policy_output.action, time_spent), keys
        )
...
def evaluate(rng_key, my_model):
    ...
    We still model clock usage:
      - our agent: MOVE_TICK_COST ticks per move
      - baseline:  BASELINE_TICK_COST ticks per move
    """
    my_player = 0
    ...
    move_ticks = jnp.int32(MOVE_TICK_COST)
