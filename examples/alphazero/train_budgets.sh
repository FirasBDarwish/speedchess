#!/bin/bash
#SBATCH --job-name=alphazero_b                   # Default job name (sweep overrides)
...
#SBATCH -o slurm-%x-%j_alphazero_b.out                 # STDOUT (%x=jobname, %j=jobid)
#SBATCH -e slurm-%x-%j_alphazero_b.err                 # STDERR
...
~/.conda/envs/torch/bin/python train_with_budgets.py
